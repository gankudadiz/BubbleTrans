...
**日期**: 2026-06-28
**版本**: v2.0
**状态**: 已实施（v2.0），后续迭代中新增右侧面板分段切换 + 当页AI总结功能，详见开发历史记录
...
**v2.0 初始设计**（已实施）：双 QTextEdit 上下排列，原文在上译文在下。

**后续迭代（2026-06-29）**：改为分段切换按钮 + 共享文本区 + 当页总结：
...
- [x] 启动程序，不再要求安装 PaddleOCR
- [x] `requirements.txt` 只剩 3 个包
- [x] 打开图片，点击「翻译当前页」，右侧原文/译文正确显示（现已改为分段切换）
- [x] 鼠标拖框选择区域，翻译结果正确
- [x] Settings 中可以切换目标语言（中文→日文），提示词变量正确注入
- [x] 切换到非中文语言后，翻译标签和输出正确
- [x] 删除 `src/engine/ocr.py` 后程序不报错
- [x] `config.json` 正确读写 `target_lang` 字段
